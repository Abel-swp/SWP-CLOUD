<!-- (keep),writer,subject,email,content,passwd 6가지를 넘겨받음. time과 ip는 여기서 따로 입력. -->
...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	<jsp:setProperty property="*" name="bean"/> <!-- writeForm에서 받은 것을 하나로 묶음(BoardBean의 전역변수와 같은 칼럼만). -->
	...
	if(cnt > 0){
		response.sendRedirect("D1_list.jsp?keep=" + keep); // keep은 모듈화된 top.jsp에서 받음
	}else{
		response.sendRedirect("D3_writeForm.jsp?keep=" + keep); // keep은 모듈화된 top.jsp에서 받음
	}
	%>