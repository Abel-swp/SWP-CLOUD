...
<head>
    ...
    var ref1 = "${cbean.ref1}";
    var ref2 = "${cbean.ref2}";
    var sdate = "${cbean.sdate}";
    var ddate = "${cbean.ddate}";
    var pdate = "${cbean.pdate}";
    var scountsu = "${cbean.scountsu}";
    var dcountsu = "${cbean.dcountsu}";
    var pcountsu = "${cbean.pcountsu}";
    var scountsi = "${cbean.scountsi}";
    var dcountsi = "${cbean.dcountsi}";
    var pcountsi = "${cbean.pcountsi}";
   	...
   	function init(f){
   	    ...
   	    var searchArr = $('#first').find("option");   // id가 '#searchValue' 인 selectBox에서 option 태그를 모두 찾아 searchArr 배열에 넣음
   	    searchArr.each(function(i){       // searchArr 배열 for문 돌림
   	     var optionValue = $(this).val();     // 해당 value값을 변수에 저장
   	     if(optionValue == ref1){            
   	      $(this).attr("selected","selected");  // selected="selected" 속성 부여	2차 카테고리를 선택하세요
   	     }
   	    });
   	    var searchArr = $('#sdate').find("option");   // id가 '#searchValue' 인 selectBox에서 option 태그를 모두 찾아 searchArr 배열에 넣음
   	    searchArr.each(function(i){       // searchArr 배열 for문 돌림
   	     var optionValue = $(this).val();     // 해당 value값을 변수에 저장
   	     if(optionValue == sdate){            
   	      $(this).attr("selected","selected");  // selected="selected" 속성 부여	2차 카테고리를 선택하세요
   	     }
   	    });
   	    var searchArr = $('#ddate').find("option");   // id가 '#searchValue' 인 selectBox에서 option 태그를 모두 찾아 searchArr 배열에 넣음
   	    searchArr.each(function(i){       // searchArr 배열 for문 돌림
   	     var optionValue = $(this).val();     // 해당 value값을 변수에 저장
   	     if(optionValue == ddate){            
   	      $(this).attr("selected","selected");  // selected="selected" 속성 부여	2차 카테고리를 선택하세요
   	     }
   	    });
   	    var searchArr = $('#pdate').find("option");   // id가 '#searchValue' 인 selectBox에서 option 태그를 모두 찾아 searchArr 배열에 넣음
   	    searchArr.each(function(i){       // searchArr 배열 for문 돌림
   	     var optionValue = $(this).val();     // 해당 value값을 변수에 저장
   	     if(optionValue == pdate){            
   	      $(this).attr("selected","selected");  // selected="selected" 속성 부여	2차 카테고리를 선택하세요
   	     }
   	    });
   	    var searchArr = $('#scountsu').find("option");   // id가 '#searchValue' 인 selectBox에서 option 태그를 모두 찾아 searchArr 배열에 넣음
   	    searchArr.each(function(i){       // searchArr 배열 for문 돌림
   	     var optionValue = $(this).val();     // 해당 value값을 변수에 저장
   	     if(optionValue == scountsu){            
   	      $(this).attr("selected","selected");  // selected="selected" 속성 부여	2차 카테고리를 선택하세요
   	     }
   	    });
	    ...
	    var searchArr = $('#dcountsu').find("option");   // id가 '#searchValue' 인 selectBox에서 option 태그를 모두 찾아 searchArr 배열에 넣음
   	    searchArr.each(function(i){       // searchArr 배열 for문 돌림
   	     var optionValue = $(this).val();     // 해당 value값을 변수에 저장
   	     if(optionValue == dcountsu){            
   	      $(this).attr("selected","selected");  // selected="selected" 속성 부여	2차 카테고리를 선택하세요
   	     }
   	    });
   	 var searchArr = $('#pcountsu').find("option");   // id가 '#searchValue' 인 selectBox에서 option 태그를 모두 찾아 searchArr 배열에 넣음
	    searchArr.each(function(i){       // searchArr 배열 for문 돌림
	     var optionValue = $(this).val();     // 해당 value값을 변수에 저장
	     if(optionValue == pcountsu){            
	      $(this).attr("selected","selected");  // selected="selected" 속성 부여	2차 카테고리를 선택하세요
	     }
	    });
	    var searchArr = $('#scountsi').find("option");   // id가 '#searchValue' 인 selectBox에서 option 태그를 모두 찾아 searchArr 배열에 넣음
   	    searchArr.each(function(i){       // searchArr 배열 for문 돌림
   	     var optionValue = $(this).val();     // 해당 value값을 변수에 저장
   	     if(optionValue == scountsi){            
   	      $(this).attr("selected","selected");  // selected="selected" 속성 부여	2차 카테고리를 선택하세요
   	     }
   	    });
   	 var searchArr = $('#dcountsi').find("option");   // id가 '#searchValue' 인 selectBox에서 option 태그를 모두 찾아 searchArr 배열에 넣음
	    searchArr.each(function(i){       // searchArr 배열 for문 돌림
	     var optionValue = $(this).val();     // 해당 value값을 변수에 저장
	     if(optionValue == dcountsi){            
	      $(this).attr("selected","selected");  // selected="selected" 속성 부여	2차 카테고리를 선택하세요
	     }
	    });
	    var searchArr = $('#pcountsi').find("option");   // id가 '#searchValue' 인 selectBox에서 option 태그를 모두 찾아 searchArr 배열에 넣음
   	    searchArr.each(function(i){       // searchArr 배열 for문 돌림
   	     var optionValue = $(this).val();     // 해당 value값을 변수에 저장
   	     if(optionValue == pcountsi){            
   	      $(this).attr("selected","selected");  // selected="selected" 속성 부여	2차 카테고리를 선택하세요
   	     }
   	    });
   	}
...
<%
String[] f_selarr = {"디자인","IT·프로그래밍","영상·사진·음향","마케팅","번역·통역","문서·글쓰기","비즈니스컨설팅","취업·투잡","운세·상담","레슨·실무교육","주문제작"};
String[][] s_selarr = {
			   {"로고·브랜딩","상세·이벤트페이지","인쇄·홍보물","제품·패키지","PPT·인포그래픽","웹·모바일디자인","블로그·SNS·썸네일","일러스트·캐리커쳐","웹툰·캐릭터·이모티콘","공간디자인","포토샵·파일변환","북·앨범디자인","캘리그라피·폰트","게임·VR","의류디자인","간판·시공","기타","파견·상주"},
			   {"워드프레스","웹사이트개발","쇼핑몰·커머스","모바일앱","임베디드HW·SW","게임","데이터베이스","데이터분석·리포트","블록체인·보안","서버·기술지원","QA·테스트","파일변환","챗봇","기타","파견·상주"},
			   {"영상촬영·편집","유튜브제작","애니메이션","3D·VR","인트로·로고","사진촬영","더빙·녹음","음악·사운드","모델·MC","기타"},
...
%>
<c:set var="arr1" value="<%=f_selarr %>"/>
<c:set var="arr2" value="<%=s_selarr %>"/>
    ...
    <!-- <div class="checkout__input"><div style="text-size:15px"><p>'<span>*</span>' 는 필수 입력 사항입니다.<p></div></div> -->
        <div class="container">
            <div class="checkout__form">
                ...
                <form action="update.fm" method="post" name="myform" enctype="multipart/form-data">
                <input type="hidden" name="num" value="${cbean.num }">
                    <div class="row">
                        <div class="col-lg-16 col-md-12">
                        	<div class="row">
								<div class="checkout__input" style="position:relative;left:15px;width:225px">
								    <p>서비스이름<span>*</span></p>
								    <input type="text" name="pname" placeholder="서비스 이름을 입력하세요" style="color:black" value="${cbean.pname }">
								</div>
								...
								<div class="checkout__input" style="position:relative;left:77.5px">
								    ...
								    <select name="ref2" id="second" style="height:46px;width:100%;">
								    	...
								    	<c:forEach var="i" begin="0" end="${fn:length(arr1)-1 }">
								    		<c:if test="${cbean.ref1 == arr1[i] }">
								    			<c:forEach var="j" begin="0" end="${fn:length(arr2[i])-1 }">
								    				<option value="${arr2[i][j]}" <c:if test="${cbean.ref2 == arr2[i][j] }">selected</c:if> >${arr2[i][j] }</option>
								    			</c:forEach>
								    		</c:if>
								    	</c:forEach>
								    </select>
                            ...
                            <table class="table table-striped" style="font-size:16px;position:relative;left:15px">
								...
								<tr>	
									...
									<td>
										<div class="checkout__input" style="width:80%;position:relative;top:12.5px">
								    		<input type="text" name="smain" placeholder="제목을 입력하세요" value="${cbean.smain}" style="width:100%;float:left;position:relative;bottom:10px;color:black">
										</div>
									</td>
									<td>
										<div class="checkout__input" style="width:80%;position:relative;top:12.5px">
								    		<input type="text" name="dmain" placeholder="제목을 입력하세요" value="${cbean.dmain }" style="width:100%;float:left;position:relative;bottom:10px;color:black">
										</div>
									</td>
									<td>
										<div class="checkout__input" style="width:80%;position:relative;top:12.5px">
								    		<input type="text" name="pmain" placeholder="제목을 입력하세요" value="${cbean.pmain }" style="width:100%;float:left;position:relative;bottom:10px;color:black">
										</div>
									</td>
								</tr>
								<tr>
									...
									<td>
										<div class="checkout__input" style="width:80%;position:relative;top:12.5px">
								    		<input type="text" name="scontent" placeholder="설명을 입력하세요" value="${cbean.scontent }" style="width:100%;float:left;position:relative;bottom:10px;color:black">
										</div>
									</td>
									<td>
										<div class="checkout__input" style="width:80%;position:relative;top:12.5px">
								    		<input type="text" name="dcontent" placeholder="설명을 입력하세요" value="${cbean.dcontent }" style="width:100%;float:left;position:relative;bottom:10px;color:black">
										</div>
									</td>
									<td>
										<div class="checkout__input" style="width:80%;position:relative;top:12.5px">
								    		<input type="text" name="pcontent" placeholder="설명을 입력하세요" value="${cbean.pcontent }" style="width:100%;float:left;position:relative;bottom:10px;color:black">
										</div>
									</td>
								</tr>
								<tr>	
									...
									<td>
										<div class="checkout__input" style="width:80%;position:relative;top:12.5px">
										<input type="text" name="svat"  placeholder="금액을 입력하세요" value="${cbean.svat }" style="width:75%;float:left;position:relative;bottom:10px;color:black"/>
										&nbsp;&nbsp;(원)
										</div>
									</td>
									<td>
										<div class="checkout__input" style="width:80%;position:relative;top:12.5px">
										<input type="text" name="dvat"  placeholder="금액을 입력하세요" value="${cbean.dvat }" style="width:75%;float:left;position:relative;bottom:10px;color:black"/>
										&nbsp;&nbsp;(원)
										</div>
									</td>
									<td>
										<div class="checkout__input" style="width:80%;position:relative;top:12.5px">
										<input type="text" name="pvat"  placeholder="금액을 입력하세요" value="${cbean.pvat }" style="width:75%;float:left;position:relative;bottom:10px;color:black"/>
										&nbsp;&nbsp;(원)
										</div>
									</td>
								</tr>
								<tr>
									...
									<td>
										<select id="sdate" name="sdate" style="width:70%;position:relative;top:3px">
									...
									<td>
										<select id="ddate" name="ddate" style="width:70%;position:relative;top:3px">
											<option value="" disabled selected>작업기간을 선택하세요</option>
											<%for(int i=1; i<=30; i++) {%>
											<option value="<%=i%>"><%=i%>일</option>
											<%}%>
									...
									<td>
										<select id="pdate" name="pdate" style="width:70%;position:relative;top:3px">
											<option value="" disabled selected>작업기간을 선택하세요</option>
											<%for(int i=1; i<=30; i++) {%>
											<option value="<%=i%>"><%=i%>일</option>
											<%}%>
								...
								<tr>
									...
									<td>
										<select id="scountsu" name="scountsu" style="width:70%;position:relative;top:3px">
									...
									<td>
										<select id="dcountsu" name="dcountsu" style="width:70%;position:relative;top:3px">
											<option value="" disabled selected>수정횟수를 선택하세요</option>
											<%for(int i=0; i<=15; i++) {%>
											<option value="<%=i%>"><%=i%>회</option>
											<%}%>
									...
									<td>
										<select id="pcountsu" name="pcountsu" style="width:70%;position:relative;top:3px">
											<option value="" disabled selected>수정횟수를 선택하세요</option>
											<%for(int i=0; i<=15; i++) {%>
											<option value="<%=i%>"><%=i%>회</option>
											<%}%>
								...
								<tr>
									...
									<td>
										<select id="scountsi" name="scountsi" style="width:70%;position:relative;top:3px">
									...
									<td>
										<select id="dcountsi" name="dcountsi" style="width:70%;position:relative;top:3px">
											<option value="" disabled selected>시안개수를 선택하세요</option>
											<%for(int i=1; i<=20; i++) {%>
											<option value="<%=i%>"><%=i%>개</option>
											<%}%>
									...
									<td>
										<select id="pcountsi" name="pcountsi" style="width:70%;position:relative;top:3px">
											<option value="" disabled selected>시안개수를 선택하세요</option>
											<%for(int i=1; i<=20; i++) {%>
											<option value="<%=i%>"><%=i%>개</option>
											<%}%>
								...
								<tr>
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="a_check">
			                                    <input type="checkbox" id="a_check" name="sorigin" <c:if test="${cbean.sorigin == 1 }">checked</c:if> >
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="b_check">
			                                    <input type="checkbox" id="b_check" name="dorigin" <c:if test="${cbean.dorigin == 1 }">checked</c:if>>
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="c_check">
			                                    <input type="checkbox" id="c_check" name="porigin" <c:if test="${cbean.porigin == 1 }">checked</c:if>>
								...
								<tr>
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="d_check">
			                                    <input type="checkbox" id="d_check" name="sdesign" <c:if test="${cbean.sdesign == 1 }">checked</c:if>>
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="e_check">
			                                    <input type="checkbox" id="e_check" name="ddesign" <c:if test="${cbean.ddesign == 1 }">checked</c:if>>
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="f_check">
			                                    <input type="checkbox" id="f_check" name="pdesign" <c:if test="${cbean.pdesign == 1 }">checked</c:if>>
								...
								<tr>
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="g_check">
			                                    <input type="checkbox" id="g_check" name="spossible" <c:if test="${cbean.spossible == 1 }">checked</c:if>>
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="h_check">
			                                    <input type="checkbox" id="h_check" name="dpossible" <c:if test="${cbean.dpossible == 1 }">checked</c:if>>
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="i_check">
			                                    <input type="checkbox" id="i_check" name="ppossible" <c:if test="${cbean.ppossible == 1 }">checked</c:if>>
								...
								<tr>
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="j_check">
			                                    <input type="checkbox" id="j_check" name="sregist" <c:if test="${cbean.sregist == 1 }">checked</c:if>>
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="k_check">
			                                    <input type="checkbox" id="k_check" name="dregist" <c:if test="${cbean.dregist == 1 }">checked</c:if>>
									...
									<td>
										<div class="checkout__input__checkbox" style="position:relative;bottom:-1.5px">
			                                <label for="l_check">
			                                    <input type="checkbox" id="l_check" name="pregist" <c:if test="${cbean.pregist == 1 }">checked</c:if>>
							...
							<div class="box">
								<textarea name="servicecontent" id="demo-message" cols="85" rows="100" style="border-bottom-color:white;box-shadow: inset 0 -1px 0 0 white;height:400px">${cbean.servicecontent }</textarea>
							</div>
							...
							<div class="col-lg-7" style="position:relative;left:-15px">
	                            <div class="checkout__input">
                        			...
                        			<div class="box">
									<textarea name="subcontent" id="demo-message" cols="57" rows="50" style="border-bottom-color:white;box-shadow: inset 0 -1px 0 0 white;height:100px;width:100%" placeholder="의뢰인에게 요청할 질문을 입력해주세요">${cbean.subcontent }</textarea>
									</div>
	                            </div>
	                        </div>
							<br><br><br>
